...
"""
This example script validates the LLaMA implementation by running
inference on a couple of prompts.

Example usage with single-GPU 7B model on slurm, with torch
compile and determinstic behavior:

  CUBLAS_WORKSPACE_CONFIG=:4096:8 srun -N 1 --gres=gpu:1 python scripts/inference.py \
                                            --model_path=~/models/7B-F/ \
...
Example usage of 13B model on 2 GPUs with Tensor Parallel:
        ...
        srun -N 1 --gres=gpu:2 torchrun --nproc_per_node=2 scripts/inference.py 
                                                            --model_path=~/models/13B-F
                                                            --tokenizer=~/models/tokenizer.model
                                                            --distributed
"""
...
def get_bos_token_id(args, tokenizer) -> int:
    """_summary_
...
def ids_for_prompt(args, tokenizer, prompt, device):
    """_summary_
    ...
    print(f"tokenizer.bos_token_id={tokenizer.bos_token_id}", flush=True)
    print(f"prompt={prompt}", flush=True)
    print(f"ids={ids}\ndevice={device}", flush=True)

    # ids = [tokenizer.bos_token_id] + ids
    ids = [get_bos_token_id(args, tokenizer)] + ids
    ids = torch.tensor(ids, dtype=torch.long, device=device)
    return ids


def print_result(result, tokenizer, local_rank, padding_kwargs):
    """_summary_
    ...
    Args:
        ...
        padding_kwargs (_type_): _description_
    """
    if local_rank != 0:
        return
...
def infer(use_cache, do_sample, tokenizer, model,
          ids, max_len,
          padding_kwargs, local_rank):
    """_summary_

    Args:
        use_cache (_type_): _description_
        do_sample (_type_): _description_
        tokenizer (_type_): _description_
    ...
    for i in range(result.shape[0]):
        print_result(result[i], tokenizer,
                     local_rank, padding_kwargs)
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If set to True, this will unfuse any fused weight"
            " modules that support the unfuse_weights method")
        )
    parser.add_argument(
        "--default_dtype",
        type=str,
        ...
        help=(
            "If set to one of the choices, overrides the model "
            "checkpoint weight format by setting the default pytorch format"
            )
    )
    parser.add_argument(
        "--compile",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Set torch.use_deterministic_algorithms? Requires "
            "env variable `CUBLAS_WORKSPACE_CONFIG=:4096:8`"
            )
    )
    parser.add_argument(
        "--distributed",
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Pad inputs to a minimum specified length. If any prompt is larger "
            "than the specified length, padding will be determined by the largest prompt"
            ),
        default=0,
    )
...
def main() -> None:
    ...
    # default_dtype = DTYPES_TO_PYT[args.default_dtype]
    ...
    if args.context_file is not None:
        ...
        with open(args.context_file, encoding="utf-8") as file:
    ...
    prompt1 = ids_for_prompt(args, tokenizer, prompt1, device)
    prompt2 = ids_for_prompt(args, tokenizer, prompt2, device)
